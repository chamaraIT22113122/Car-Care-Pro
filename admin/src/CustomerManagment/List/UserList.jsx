import React, { useEffect, useState } from "react";
import axios from "axios";
import { Link, useNavigate } from "react-router-dom";
import "../../Styles/table.css";
import { assets } from "../../assets/assets";
import { toast } from 'react-toastify';
import './UserList.css';
import jsPDF from "jspdf";
import autoTable from 'jspdf-autotable';
import Swal from 'sweetalert2';

const UserList = () => {
  const [users, setUsers] = useState([]);
  const [selectedUsers, setSelectedUsers] = useState([]);
  const [totalUsers, setTotalUsers] = useState(0);
  const [totalActiveUsers, setTotalActiveUsers] = useState(0);
  const [dateRange, setDateRange] = useState({ start: '', end: '' });
  const [loading, setLoading] = useState(false);
  const [statusFilter, setStatusFilter] = useState('all'); // 'all', 'active', 'inactive'

  const navigate = useNavigate();

  useEffect(() => {
    fetchUsers();
  }, [dateRange.start, dateRange.end, statusFilter]); // Track status filter as well

  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:4000/api/user");
      let filteredUsers = response.data.filter(user => {
        const registrationDate = new Date(user.registrationDate);
        const startDate = dateRange.start ? new Date(dateRange.start) : null;
        const endDate = dateRange.end ? new Date(dateRange.end) : null;

        return (
          (!startDate || registrationDate >= startDate) &&
          (!endDate || registrationDate <= endDate)
        );
      });

      // Filter based on active/inactive status
      if (statusFilter === 'active') {
        filteredUsers = filteredUsers.filter(user => user.active);
      } else if (statusFilter === 'inactive') {
        filteredUsers = filteredUsers.filter(user => !user.active);
      }

      setUsers(filteredUsers);
      setTotalUsers(filteredUsers.length);
      setTotalActiveUsers(filteredUsers.filter(user => user.active).length);
    } catch (error) {
      console.error("Error fetching users", error);
      toast.error('Failed to fetch users');
    }
    setLoading(false);
  };

  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You won't be able to revert this!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, delete it!'
    });

    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:4000/api/user/delete/${id}`);
        fetchUsers();
        Swal.fire('Deleted!', 'The customer has been deleted.', 'success');
      } catch (error) {
        console.error("Error deleting user", error);
        Swal.fire('Error!', 'Failed to delete user.', 'error');
      }
    }
  };

  const handleBulkDelete = async () => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: "You won't be able to revert this for selected users!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, delete selected users!'
    });

    if (result.isConfirmed) {
      try {
        for (let userId of selectedUsers) {
          await axios.delete(`http://localhost:4000/api/user/delete/${userId}`);
        }
        fetchUsers();
        Swal.fire('Deleted!', 'Selected customers have been deleted.', 'success');
      } catch (error) {
        console.error("Error deleting users", error);
        Swal.fire('Error!', 'Failed to delete selected users.', 'error');
      }
    }
  };

  const handleToggleActive = async (id, currentStatus) => {
    const newStatus = !currentStatus;
    try {
      const response = await axios.patch(`http://localhost:4000/api/users/status/${id}`, { active: newStatus });
      if (response.status === 200) {
        fetchUsers();
        Swal.fire(
          'Success!',
          `User has been ${newStatus ? 'activated' : 'deactivated'} successfully!`,
          'success'
        );
      }
    } catch (error) {
      console.error("Error toggling user status:", error);
      Swal.fire(
        'Error!',
        `Failed to ${newStatus ? 'activate' : 'deactivate'} user`,
        'error'
      );
    }
  };

  const handleBulkToggleActive = async (newStatus) => {
    try {
      for (let userId of selectedUsers) {
        await axios.patch(`http://localhost:4000/api/users/status/${userId}`, { active: newStatus });
      }
      fetchUsers();
      Swal.fire('Success!', `Selected users have been ${newStatus ? 'activated' : 'deactivated'}.`, 'success');
    } catch (error) {
      console.error("Error updating users' statuses", error);
      Swal.fire('Error!', 'Failed to update selected users.', 'error');
    }
  };

  const generatePDF = () => {
    const doc = new jsPDF();

    doc.setFontSize(18);
    doc.text('Car Care Pro', 105, 20, null, null, 'center');
    doc.setFontSize(12);
    doc.text('Yakkala, Sri Lanka', 105, 30, null, null, 'center');
    doc.text(`Total Customers: ${users.length}`, 14, 40);
    doc.text(`Total Active Customers: ${users.filter(user => user.active).length}`, 14, 50);
    doc.text(`Date Range: ${dateRange.start || 'N/A'} to ${dateRange.end || 'N/A'}`, 14, 60);

    const tableColumn = ["Name", "Email", "Phone", "Address", "Username", "Status"];
    const tableRows = users.map(user => [
      user.name,
      user.email,
      user.phone,
      `${user.address1} ${user.address2 || ''} ${user.hometown}`,
      user.username,
      user.active ? "Active" : "Inactive"
    ]);

    autoTable(doc, {
      startY: 70,
      head: [tableColumn],
      body: tableRows,
      styles: { fontSize: 10, cellPadding: 2 },
      headStyles: { fillColor: [0, 122, 255] },
    });

    doc.save('customer_report.pdf');
  };

  const handleSelectAll = (event) => {
    if (event.target.checked) {
      setSelectedUsers(users.map(user => user._id));
    } else {
      setSelectedUsers([]);
    }
  };

  const handleSelectUser = (userId) => {
    setSelectedUsers(prevSelected => {
      if (prevSelected.includes(userId)) {
        return prevSelected.filter(id => id !== userId);
      } else {
        return [...prevSelected, userId];
      }
    });
  };

  return (
    <div className="list add flex-col" style={{ width: "950px", borderRadius: "10px" }}>
      <div className="list-header">
        <h2>Customer List</h2>
        <Link className="add-item-btn" to={"/customer/add"}>Add Customer</Link>
      </div>

      <div className="date-range">
        <label htmlFor="start-date">Start Date:</label>
        <input
          type="date"
          id="start-date"
          value={dateRange.start}
          onChange={(e) => setDateRange({ ...dateRange, start: e.target.value })}
        />
        <label htmlFor="end-date">End Date:</label>
        <input
          type="date"
          id="end-date"
          value={dateRange.end}
          onChange={(e) => setDateRange({ ...dateRange, end: e.target.value })}
        />
        <button className="add-inventory-btn" onClick={generatePDF}>
          Generate Report
        </button>
      </div>

      <div className="filter">
        <label htmlFor="status-filter">Filter by Status:</label>
        <select
          id="status-filter"
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
        >
          <option value="all">All</option>
          <option value="active">Active</option>
          <option value="inactive">Inactive</option>
        </select>
      </div>

      <div className="bulk-actions">
        <button 
          className="bulk-action-btn" 
          onClick={handleBulkDelete} 
          disabled={selectedUsers.length === 0}
        >
          Bulk Delete
        </button>
        <button 
          className="bulk-action-btn" 
          onClick={() => handleBulkToggleActive(true)} 
          disabled={selectedUsers.length === 0}
        >
          Bulk Activate
        </button>
        <button 
          className="bulk-action-btn" 
          onClick={() => handleBulkToggleActive(false)} 
          disabled={selectedUsers.length === 0}
        >
          Bulk Deactivate
        </button>
      </div>

      <div className="stats">
        <div className="stat-card">
          <h3>Total Customers</h3>
          <p>{totalUsers}</p>
        </div>
        <div className="stat-card">
          <h3>Total Active Customers</h3>
          <p>{totalActiveUsers}</p>
        </div>
      </div>

      {loading ? (
        <p>Loading customers...</p>
      ) : (
        <table className="list-table">
          <thead>
            <tr>
              <th>
                <input 
                  type="checkbox" 
                  onChange={handleSelectAll} 
                  checked={selectedUsers.length === users.length}
                />
              </th>
              <th>Name</th>
              <th>Email</th>
              <th>Username</th>
              <th>Phone</th>
              <th>Address</th>
              <th>Status</th>
              <th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user) => (
              <tr key={user._id}>
                <td>
                  <input
                    type="checkbox"
                    checked={selectedUsers.includes(user._id)}
                    onChange={() => handleSelectUser(user._id)}
                  />
                </td>
                <td>{user.name}</td>
                <td>{user.email}</td>
                <td>{user.username}</td>
                <td>{user.phone}</td>
                <td>
                  {user.address1}
                  <br />
                  {user.address2 && <>{user.address2}<br /></>}
                  {user.hometown}
                </td>
                <td>
                  <button
                    className={`status-btn ${user.active ? 'active' : 'inactive'}`}
                    onClick={() => handleToggleActive(user._id, user.active)}
                  >
                    {user.active ? "Active" : "Inactive"}
                  </button>
                </td>
                <td className="actions">
                  <button className="delete-btn" onClick={() => handleDelete(user._id)}>
                    <img src={assets.delete_icon} alt="Delete Icon" className="icon" />
                  </button>
                  <button className="update-btn" onClick={() => navigate(`/customer/edit/${user._id}`)}>
                    <img src={assets.edit_icon} alt="Edit Icon" className="icon" />
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      )}
    </div>
  );
};

export default UserList;
