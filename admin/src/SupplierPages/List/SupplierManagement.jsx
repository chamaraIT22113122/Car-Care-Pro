...
const SupplierManagement = () => {
  ...
  const generateReport = async () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("Company Name: Car care pro", 10, 10);
    doc.text("Address: Yakkala, Sri Lanka", 10, 20);
    doc.text("Generated By: Employee Manager", 10, 30);
    doc.text(`Total Employeee: ${totalSuppliers}`, 10, 50);
    ...
    doc.autoTable({
      head: [
        ["Employee Name", "Status", "Work Hours", "Per Hour", "Total Salary"],
      ],
      body: filteredOrders.map((order) => {
        ...
        return [
          supplier?.name || "Employee not found",
          order.productName,
          order.quantity,
  ...
  const handleDeleteSupplier = async (id) => {
    if (window.confirm("Are you sure you want to delete this Employee?")) {
      ...
      if (response.ok) {
        toast.success('Employee removed successfull!')
  ...
  const handleDeleteOrder = async (id) => {
    if (window.confirm("Are you sure you want to delete this sallary?")) {
      const response = await fetch(
        `http://localhost:4000/api/supplier-orders/delete/${id}`,
        { method: "DELETE" }
      );
      if (response.ok) {
        toast.success('Sallary Removed Success!')
        setOrders(orders.filter((order) => order._id !== id));
      } else {
        showNotification("Failed to delete Sallary!", "danger");
      }
    }
  };
  ...
  return (
    <div className="container" style={{ width: "1000px" }}>
      <div className="list-header">
        ...
        <h1 style={{ marginLeft: "10px" }}>Employee Management</h1>
